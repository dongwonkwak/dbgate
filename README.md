...
**환경**: MySQL 8.0, 4 스레드, 30초 실행, 10K 행 × 4 테이블

| 워크로드 | 모드 | TPS | QPS | P95(ms) | P99(ms) | 오버헤드 |
|---------|------|-----|-----|---------|---------|---------|
| **oltp_read_only** | 직접연결 | 2,847 | 45,552 | 2.81 | 4.15 | - |
| | 프록시 | 2,756 | 44,096 | 3.04 | 4.62 | **+8.2%** ✓ |
| **oltp_read_write** | 직접연결 | 1,563 | 31,260 | 5.21 | 8.94 | - |
| | 프록시 | 1,502 | 30,040 | 5.68 | 9.47 | **+9.0%** ✓ |

**성능 목표 달성**:
- ✓ P95 오버헤드 <= 10% (read-only: 8.2%, read-write: 9.0%)
- ✓ 프록시 TPS >= 1,000 (read-only: 2,756, read-write: 1,502)
