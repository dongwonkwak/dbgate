...
##### 3. policy_versions (DON-50)

저장된 정책 스냅샷 버전 목록을 조회합니다.
...
```json
{
  ...
  "payload": {
    ...
    "versions": [
      {
        "version": 5,
        "timestamp": "20260304T103500Z",
        "rules_count": 24,
        "hash": "abc123def456..."
      },
      {
        "version": 4,
        "timestamp": "20260304T090000Z",
        "rules_count": 20,
        "hash": "789xyz..."
      }
    ]
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `current` | uint64 | 현재 활성 정책의 버전 번호 (PolicyEngine::current_version()) |
| `versions` | array | 저장된 버전 메타데이터 목록 (최신 버전 먼저) |
| `versions[].version` | uint64 | 버전 번호 (단조 증가) |
| `versions[].timestamp` | string | 스냅샷 저장 시각 (ISO 8601 compact UTC, 예: `"20260304T103500Z"`) |
| `versions[].rules_count` | uint32 | 해당 버전의 access_control 규칙 수 |
| `versions[].hash` | string | 원본 YAML 파일의 SHA-256 hex digest (무결성 확인용, 실패 시 빈 문자열) |

**응답** (version_store 미주입):
```json
{
  "ok": false,
  "error": "not implemented",
...
**용도**:
- 현재 활성 정책 버전 확인
- 사용 가능한 롤백 대상 버전 목록 조회
- 정책 변경 이력 추적

---

##### 4. policy_rollback (DON-50)

지정된 버전의 정책 스냅샷으로 롤백합니다.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `target_version` | uint64 | ✓ | 롤백할 대상 버전 번호 |
...
{
  "ok": false,
  "error": "policy_version_store: version 3 not found"
}
```
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `rolled_back_to` | uint64 | 롤백된 버전 번호 |
| `previous_version` | uint64 | 롤백 직전 활성 버전 번호 |
| `rules_count` | uint32 | 롤백된 정책의 access_control 규칙 수 |

**fail-close 보장**:
- `target_version` 필드 누락 → `ok:false`
- 버전 미발견 또는 스냅샷 파일 없음 → `ok:false`, 현재 정책 유지
- 로드 실패 → `ok:false`, 현재 정책 유지

**용도**:
- 정책 배포 실패 후 이전 버전으로 즉시 복구
- 운영 장애 대응 (Go CLI `dbgate policy rollback --version 3`)

---

##### 5. policy_reload (DON-50)

정책 설정 파일을 리로드하고 스냅샷을 저장합니다.
...
```json
{
  "ok": true,
  "payload": {
    "reloaded_at": "2026-03-04T10:35:20Z",
    "rules_count": 24,
    "version": 6,
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `reloaded_at` | string | 리로드 완료 시각 (ISO 8601 UTC, 예: `"2026-03-04T10:35:20Z"`) |
| `rules_count` | uint32 | 새로 로드된 정책의 access_control 규칙 수 |
| `version` | uint64 | 새로 부여된 정책 버전 번호 |
| `message` | string | 사람이 읽을 수 있는 완료 메시지 |

**fail-close 보장**:
- YAML 파싱 실패 → `ok:false`, 현재 정책 유지
- version_store 미주입 → `ok:false` (501)
- 스냅샷 저장 실패 → 경고 로그 출력 후 리로드 계속 진행 (스냅샷 실패가 정책 리로드를 막지 않음)

**SIGHUP 연동**:
`SIGHUP` 시그널 수신 시 ProxyServer 내부적으로 동일한 흐름(파일 로드 → 스냅샷 저장 → 버전 갱신)을 수행합니다.
...
**용도**:
- 정책 파일 변경 후 수동 리로드 (SIGHUP 보완)
- 설정 적용 실패 시 명시적 재시도
- 운영 편의성 (Go CLI `dbgate policy reload`)
...
### 시나리오 3: Policy Versions 조회

```
Go CLI                              C++ UdsServer
   │                                    │
   ├─ 요청 송신 ─────────────────────────→
   │  {"command":"policy_versions","version":1}
   │
   │                       (PolicyVersionStore::list_versions())
   ...
   │    "current":5,
   │    "versions":[
   │      {"version":5,"timestamp":"20260304T103500Z",
   │       "rules_count":24,"hash":"abc123..."},
   │      {"version":4,"timestamp":"20260304T090000Z",
   │       "rules_count":20,"hash":"789xyz..."}
   │    ]
   │  }}
   │
   ├─ 버전 목록 출력
...
### 시나리오 4: Policy Rollback

```
Go CLI                              C++ UdsServer
   │                                    │
   ├─ 요청 송신 ─────────────────────────→
   │  {"command":"policy_rollback","version":1,
   │   "payload":{"target_version":3}}
   │
   │                  (PolicyVersionStore::load_snapshot(3))
   │                  (PolicyEngine::reload(config, 3))
   │
   │←─ 응답 송신 ─────────────────────────┤
   │  {"ok":true,"payload":{
   │    "rolled_back_to":3,
...
**DON-50 구현 완료**:
- `policy_versions` 커맨드 구현
- `policy_rollback` 커맨드 구현
- `policy_reload` 커맨드 실제 구현 (501 placeholder 해제)
...
| 1.3 | 2026-03-04 | DON-50: `policy_versions`, `policy_rollback` 커맨드 추가. `policy_reload` 커맨드 실제 구현 (501 placeholder 해제), 응답에 `version` 필드 추가. ProxyServer 연동으로 SIGHUP 시 스냅샷 자동 저장. |
