...
이 에이전트는 `docs/`만 수정하므로 CI의 `check-doc-impact.sh` 체크 대상이 아니다.
CLAUDE.md의 "Doc Impact 규칙"은 구현 에이전트(코드 수정 시 문서 업데이트 의무)에 적용되며,
technical-writer는 문서 자체가 산출물이므로 해당 규칙의 적용을 받지 않는다.
