...
TEST(MysqlPacketParse, SuccessNormalPacket) {
    ...
    std::vector<std::uint8_t> data = {
        0x04,
        0x00,
        0x00,  // payload length = 4 (LE)
        0x00,  // sequence_id = 0
        0x03,
        0x53,
        0x45,
        0x4C  // payload: 0x03 = COM_QUERY, "SEL"
    };

    const auto result = MysqlPacket::parse(std::span<const std::uint8_t>{data});
...
TEST(MysqlPacketParse, ErrorTooShort) {
    const std::vector<std::uint8_t> data = {0x01, 0x00};  // 2바이트만
...
TEST(MysqlPacketParse, ErrorIncompletePayload) {
    ...
    std::vector<std::uint8_t> data = {
        0x0A,
        0x00,
        0x00,  // payload length = 10
        0x01,  // sequence_id = 1
        0x03,
        0x41,
        0x42  // payload 3바이트만 제공
    };
...
TEST(MysqlPacketType, EofPacket) {
    ...
    const std::vector<std::uint8_t> data = {0x05, 0x00, 0x00, 0x01, 0xFE, 0x00, 0x00, 0x02, 0x00};
...
TEST(MysqlPacketType, OkPacket) {
    const std::vector<std::uint8_t> data = {0x01, 0x00, 0x00, 0x02, 0x00};
...
TEST(MysqlPacketMakeError, ErrorPacketFormat) {
    ...
    const MysqlPacket pkt = MysqlPacket::make_error(error_code, message, seq_id);
...
TEST(MysqlPacketSerialize, RoundTrip) {
    ...
    const auto parse_result = MysqlPacket::parse(std::span<const std::uint8_t>{original});
...
TEST(MysqlPacketSerialize, HeaderFormat) {
    ...
    const std::vector<std::uint8_t> data = {0x03, 0x00, 0x00, 0x01, 0xFF, 0x01, 0x02};
    ...
    // 3바이트 LE length
    const auto len = static_cast<std::uint32_t>(serialized[0]) |
...
TEST(ExtractCommand, ComQueryExtractsQuery) {
    ...
    // wire format
    const auto len = static_cast<std::uint32_t>(payload_bytes.size());
    std::vector<std::uint8_t> data;
    ...
    const auto parse_result = MysqlPacket::parse(std::span<const std::uint8_t>{data});
    ASSERT_TRUE(parse_result.has_value());
...
TEST(MysqlPacketMakeError, ParsedBackAsError) {
    const MysqlPacket err_pkt = MysqlPacket::make_error(1064, "syntax error", 1);
    const auto serialized = err_pkt.serialize();

    const auto re_parsed = MysqlPacket::parse(std::span<const std::uint8_t>{serialized});
...
TEST(MysqlPacketType, FELargePayloadNotEof) {
    ...
    std::vector<std::uint8_t> payload_body = {
        ...
        0x07,
        0x08,
        0x09  // 5바이트 more (total: 1+9 = 10 bytes payload)
    };
    const auto len = static_cast<std::uint32_t>(payload_body.size());
...
TEST(MysqlPacketMakeError, SequenceIdZero) {
    const MysqlPacket pkt = MysqlPacket::make_error(2003, "Can't connect", 0);
    EXPECT_EQ(pkt.sequence_id(), 0);
    EXPECT_EQ(pkt.type(), PacketType::kError);
...
TEST(MysqlPacketMakeError, EmptyMessage) {
    const MysqlPacket pkt = MysqlPacket::make_error(1000, "", 1);
    EXPECT_EQ(pkt.type(), PacketType::kError);
