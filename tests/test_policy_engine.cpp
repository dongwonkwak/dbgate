...
ParsedQuery make_query(SqlCommand cmd = SqlCommand::kSelect,
...
TEST(PolicyEngine, BlockPattern_Sleep) {
    const PolicyEngine engine(make_basic_config());
    const auto query = make_query(SqlCommand::kSelect, {}, "SELECT SLEEP(5)");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyEngine, BlockPattern_CaseInsensitive) {
    ...
    const PolicyEngine engine(make_basic_config());
    const auto query = make_query(SqlCommand::kSelect, {}, "select * from t union select 1");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyEngine, BlockPattern_InvalidRegex_Skipped) {
    ...
    const auto query = make_query(SqlCommand::kSelect, {}, "SELECT 1 UNION SELECT 2");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyEngine, Procedure_PrepareBlocked_DynamicSql) {
    ...
    auto cfg = make_basic_config();
    cfg->access_control[0].allowed_operations = {
        "SELECT", "INSERT", "UPDATE", "PREPARE", "EXECUTE"};
    const PolicyEngine engine(cfg);
...
TEST(PolicyEngine, SchemaAccess_InformationSchema_Blocked) {
    ...
    const auto query = make_query(
        SqlCommand::kSelect, {"information_schema"}, "SELECT * FROM information_schema.tables");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyEngine, SchemaAccess_Mysql_Blocked) {
    ...
    const auto query = make_query(SqlCommand::kSelect, {"mysql"}, "SELECT * FROM mysql.user");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyEngine, SchemaAccess_DotNotation_Mysql_Blocked) {
    ...
    const auto query = make_query(SqlCommand::kSelect, {"mysql.user"}, "SELECT * FROM mysql.user");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyEngine, SchemaAccess_DotNotation_NormalTable_NotBlocked) {
    ...
    const auto query = make_query(SqlCommand::kSelect, {"mydb.users"}, "SELECT id FROM mydb.users");
    const auto result = engine.evaluate(query, make_session());
...
TEST(PolicyLoader, LoadValidFile_Succeeds) {
    ...
    const std::string tmp_path = "/tmp/test_policy_valid.yaml";
    {
        ...
        ASSERT_NE(f, nullptr);  // NOLINT(clang-analyzer-unix.Stream)
...
data_protection:
  ...
  block_schema_access: true
                   ...
                   f);   // NOLINT(cert-err33-c)
        std::fclose(f);  // NOLINT(cert-err33-c,cppcoreguidelines-owning-memory)
    }
    ...
    std::remove(tmp_path.c_str());  // NOLINT(cert-err33-c)
}
...
TEST(PolicyLoader, LoadFile_InvalidYaml_ReturnsError) {
    ...
    const std::string tmp_path = "/tmp/test_policy_invalid.yaml";
    {
        ...
        std::FILE* f = std::fopen(tmp_path.c_str(), "w");
        ASSERT_NE(f, nullptr);                       // NOLINT(clang-analyzer-unix.Stream)
        std::fputs("{ invalid yaml: [unclosed", f);  // NOLINT(cert-err33-c)
        std::fclose(f);  // NOLINT(cert-err33-c,cppcoreguidelines-owning-memory)
    }
...
TEST(PolicyEngine, CidrMatch_Slash32_ExactIp) {
    ...
    // 정확히 일치하는 IP
    const auto result1 =
        engine.evaluate(make_query(SqlCommand::kSelect, {"users"}, "SELECT id FROM users"),
...
TEST(PolicyEngine, KnownFalseNegative_CommentSplit) {
    ...
    (void)result;
    SUCCEED()
        << "Comment-split bypass (UN/**/ION SELECT) is a known false negative in block_patterns.";
}
...
TEST(PolicyEngine, KnownFalseNegative_EncodingBypass) {
    const PolicyEngine engine(make_basic_config());
    const auto query =
        make_query(SqlCommand::kSelect, {}, "SELECT CHAR(85,78,73,79,78,32,83,69,76,69,67,84)");
    const auto result = engine.evaluate(query, make_session());
    (void)result;
...
TEST(PolicyEngine, BlockedOverridesAllowed_UpdateDenied) {
    ...
    const auto select_result = engine.evaluate(
        make_query(SqlCommand::kSelect, {"users"}, "SELECT id FROM users"), make_session());
...
TEST(PolicyEngine, BlockedOverridesAllowed_InsertDenied_WhenBlocked) {
    ...
    // SELECT 는 정상 허용
    const auto select_result = engine.evaluate(
        make_query(SqlCommand::kSelect, {"users"}, "SELECT * FROM users"), make_session());
    EXPECT_EQ(select_result.action, PolicyAction::kAllow);
}
...
TEST(PolicyEngine, EmptyAccessControl_BlocksMultipleQueryTypes) {
    ...
    for (const auto& [cmd, sql] : cmds) {
        const auto result =
            engine.evaluate(make_query(cmd, {}, sql), make_session("anyuser", "1.2.3.4"));
        EXPECT_EQ(result.action, PolicyAction::kBlock)
...
TEST(PolicyEngine, Reload_MultipleCalls_LastConfigApplied) {
    ...
    // admin SELECT → allow
    {
        const auto result = engine.evaluate(make_query(SqlCommand::kSelect, {"users"}, "SELECT 1"),
